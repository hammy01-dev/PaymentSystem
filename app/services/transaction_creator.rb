      ...
      for i in 0..data.length-1
        if result[data[i]["user_id"]]
          if data[i]["usage"].to_i > data[i]["max_unit_limit"].to_i
            result[data[i]["user_id"]] = (data[i]["usage"].to_i - data[i]["max_unit_limit"].to_i) *data[i]["unit_price"].to_i
          end
        else
          if data[i]["usage"].to_i > data[i]["max_unit_limit"].to_i
            result[data[i]["user_id"]] = (data[i]["usage"] - data[i]["max_unit_limit"])*data[i]["unit_price"].to_i
          else
            result[data[i]["user_id"]] = data[i]["monthly_fee"]
          end
        end
      end
      result
